...
> https://doi.org/10.5281/zenodo.XXXXXXX *(replace with the published DOI)*

See [`data_dictionary.md`](data_dictionary.md) in the Zenodo record for a per-file description. *(Data files are large; for this GitHub repository, download them from Zenodo and place them under `MMHID/`.)*

## Repository Structure

```
.
├── DG_Unet1_none.ipynb            # digestive gland, plain U-Net
├── DG_Unet2_vgg.ipynb             # digestive gland, VGG16 encoder
├── DG_Unet3_resnet34.ipynb        # digestive gland, ResNet34 encoder ★
├── DG_Unet4_efficientnetb0.ipynb  # digestive gland, EfficientNet-B0 encoder
├── G_Unet1_none.ipynb             # gill filament, plain U-Net
├── G_Unet2_vgg.ipynb              # gill filament, VGG16 encoder
├── G_Unet3_resnet34.ipynb         # gill filament, ResNet34 encoder ★
├── G_Unet4_efficientnetb0.ipynb   # gill filament, EfficientNet-B0 encoder
...
Requirements: NVIDIA GPU with ≥8 GB VRAM (CUDA 11+), ≥16 GB RAM. Full dependency list in [`requirements.txt`](requirements.txt).
...
U-Net from `segmentation_models` with a ResNet34 encoder, input `(512, 512, 3)`, binary cross-entropy loss, Adam optimizer, batch size 50, 100 epochs, 40% validation split (~24.5 M parameters). Full architecture and training details: [`TECHNICAL_DOCUMENTATION.md`](TECHNICAL_DOCUMENTATION.md).
...
> *(to be completed)* Mussel-Histopathradiomics: U-Net-Based Automated Segmentation of Mussel Gill Filament Epithelium and Digestive Gland Lumens. Zenodo. https://doi.org/10.5281/zenodo.XXXXXXX
