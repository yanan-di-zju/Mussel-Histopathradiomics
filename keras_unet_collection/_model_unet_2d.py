...
def unet_2d(input_size, filter_num, n_labels, stack_num_down=2, stack_num_up=2,
            ...
            backbone=None, weights='imagenet', freeze_backbone=True, freeze_batch_norm=True, name='unet'):
    '''
    U-net with an optional ImageNet-trained bakcbone.# 带有可选择的imagenet预训练主干网络的U-net
    ...
    Input
    ----------
        input_size: the size/shape of network input, e.g., `(128, 128, 3)`.输入图片大小
        filter_num: a list that defines the number of filters for each \ 卷积核数量，定义每层卷积核数量的列表
                    down- and upsampling levels. e.g., `[64, 128, 256, 512]`.
                    The depth is expected as `len(filter_num)`. 网络深度预期是len（filter_num）,定义好各层卷积核数量之后也就确定了网络深度
        n_labels: number of output labels. # 输出标签的数量？
        stack_num_down: number of convolutional layers per downsampling level/block. 每个下采样块的卷积层数
        stack_num_up: number of convolutional layers (after concatenation) per upsampling level/block.每个上采样块的卷积层数（连接后）
        activation: one of the `tensorflow.keras.layers` or `keras_unet_collection.activations` interfaces, e.g., 'ReLU'.激活函数
        output_activation: one of the `tensorflow.keras.layers` or `keras_unet_collection.activations` interface or 'Sigmoid'.最后一层激活函数
                           Default option is 'Softmax'.默认选项是softmax
                           if None is received, then linear activation is applied.如果None，就用线性激活
        batch_norm: True for batch normalization. 批归一化
        pool: True or 'max' for MaxPooling2D.池化 True 或 'max'是最大池化
              'ave' for AveragePooling2D. 'ave'是平均池化
              False for strided conv + batch norm + activation. False 是没有池化，仅仅是 卷积+批归一化+激活函数
        unpool: True or 'bilinear' for Upsampling2D with bilinear interpolation. 上采样：True或者‘bilinear’是采用bilinear interpolation进行上采样
                'nearest' for Upsampling2D with nearest interpolation.‘nearest’是用nearest interpolation进行上采样
                False for Conv2DTranspose + batch norm + activation.  False 是没有上采样 只是 卷积转置 + 批归一化 + 激活函数
        name: prefix of the created keras model and its layers. 创建的 keras 模型及其层的前缀。
        
        ---------- (keywords of backbone options) ----------主干网络选项关键字
        backbone_name: the bakcbone model name. Should be one of the `tensorflow.keras.applications` class.
        主干网络名字        None (default) means no backbone. None表示没有主干网络
                       Currently supported backbones are: 目前支持的骨干网是
                       (1) VGG16, VGG19
                       (2) ResNet50, ResNet101, ResNet152
                       (3) ResNet50V2, ResNet101V2, ResNet152V2
                       (4) DenseNet121, DenseNet169, DenseNet201
        权重             (5) EfficientNetB[0-7]
        weights: one of None (random initialization), 'imagenet' (pre-training on ImageNet), 
                 or the path to the weights file to be loaded.
             None：随机初始化，‘imagenet’再imagenet上预训练，或添加要加载权重文件的路径
        freeze_backbone: True for a frozen backbone. True代表冻结主干网络
        freeze_batch_norm: False for not freezing batch normalization layers.
        冻结批归一化层   False代表不冻结批归一化层          
